...
Enumerating and reading the SQLite-backed agents — opencode, Cursor IDE and Copilot CLI — needs the optional `sqlite3` gem; every other agent works with the standard library alone.

Read a session's messages and token usage (Claude, Codex, Amp, opencode, pi, Gemini CLI, Copilot CLI, Qwen and Grok):

```ruby
reader = AgentSessions.read(session)
...
- 0.4: SQLite readers via optional `sqlite3` (opencode landed early, in 0.3; Cursor remains)
