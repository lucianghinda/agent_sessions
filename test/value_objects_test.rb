...
class ValueObjectsTest < Minitest::Test
  def test_error_hierarchy
    assert_operator Agent::Sessions::UnknownAgent, :<, Agent::Sessions::Error
    assert_operator Agent::Sessions::MissingDependency, :<, Agent::Sessions::Error
    assert_operator Agent::Sessions::UnreadableStore, :<, Agent::Sessions::Error
    assert_operator Agent::Sessions::UnsupportedFormat, :<, Agent::Sessions::Error
  end

  def test_env_override_active
    assert Agent::Sessions::EnvOverride.new(name: "CODEX_HOME", value: "/x").active?
    refute Agent::Sessions::EnvOverride.new(name: "CODEX_HOME", value: nil).active?
    refute Agent::Sessions::EnvOverride.new(name: "CODEX_HOME", value: "").active?
  end

  def test_location_exists_and_lists_glob_files
    Dir.mktmpdir do |dir|
      FileUtils.touch(File.join(dir, "a.jsonl"))
      location = Agent::Sessions::Location.new(kind: :sessions, path: dir, format: :jsonl, glob: "*.jsonl")
  ...
  def test_single_file_location_lists_itself
    Dir.mktmpdir do |dir|
      ...
      location = Agent::Sessions::Location.new(kind: :history, path: path, format: :jsonl, single_file: true)
      assert_equal [path], location.files
    end
  end

  def test_single_file_location_lists_nothing_when_absent
    location = Agent::Sessions::Location.new(
  ...
  def test_directory_without_glob_lists_nothing_and_says_so
    Dir.mktmpdir do |dir|
      ...
      location = Agent::Sessions::Location.new(kind: :legacy, path: dir, format: :json)
      assert_empty location.files
  ...
  def test_enumerable_covers_globs_and_single_files
    glob = Agent::Sessions::Location.new(kind: :sessions, path: "/x", format: :jsonl, glob: "*.jsonl")
    file = Agent::Sessions::Location.new(kind: :history, path: "/x/h.jsonl", format: :jsonl, single_file: true)
    assert glob.enumerable?
    assert file.enumerable?
  end

  def test_location_defaults_keep_glob_optional
    location = Agent::Sessions::Location.new(kind: :legacy, path: "/x", format: :json)
    assert_nil location.glob
    refute location.single_file
  end

  def test_check_predicates
    check = Agent::Sessions::Check.new(agent: :claude, status: :pass, claim: "c", detail: "d")
    assert check.pass?
    refute Agent::Sessions::Check.new(agent: :claude, status: :fail, claim: "c", detail: "d").pass?
  end
  ...
  def test_location_files_survives_a_symlink_loop
    Dir.mktmpdir do |dir|
      ...
      location = Agent::Sessions::Location.new(kind: :sessions, path: looped, format: :jsonl, glob: "*.jsonl")
      assert_empty location.files
    end
  end
  ...
  def test_location_files_treats_path_metacharacters_literally
    Dir.mktmpdir do |dir|
      ...
      location = Agent::Sessions::Location.new(kind: :sessions, path: weird, format: :jsonl, glob: "*.jsonl")
